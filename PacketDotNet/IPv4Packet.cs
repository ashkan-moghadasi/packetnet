...
#pragma warning restore 0169, 0649
    ...
    public IPOption IPOption { get; set; }

    /// <summary>
    /// Construct an instance by values
    /// </summary>
    public IPv4Packet(IPAddress sourceAddress, IPAddress destinationAddress, IPOption ipOption = null)

    {
        this.IPOption = ipOption;
        // allocate memory for this packet
        var length = ipOption == null ? IPv4Fields.HeaderLength : IPv4Fields.HeaderLength + ipOption.Length;
        var headerBytes = new byte[length];

        Header = new ByteArraySegment(headerBytes,
                                      0,
                                      length);
        if (ipOption != null)
        {
            ipOption.Bytes.CopyTo(Header.Bytes,
                                  IPv4Fields.OptionsPosition);
        }

        // set some default values to make this packet valid
        PayloadLength = 0;
        HeaderLength =
            (HeaderMinimumLength + (ipOption?.Length ?? 0)) /
            4; // NOTE: HeaderLength is the number of 32bit words in the header
    ...
    public IPv4Packet(ByteArraySegment byteArraySegment)
    {
        ...
        Log.DebugFormat("IPv4Packet HeaderLength {0}",
                        HeaderLength);
        Log.DebugFormat("header {0}",
                        Header);

        //IP Option Check
        if (HeaderLength > 5)
        {
            var optionType = Header[IPv4Fields.OptionsPosition];
            var optionLength = Header[IPv4Fields.OptionsPosition + 1];

            IPOption = optionType switch
            {
                149 => IPOption = Rfc1770IPOption.Create(Header.Bytes.ToList().Skip(IPv4Fields.OptionsPosition).Take(optionLength).ToArray()),
                _ => throw new NotImplementedException("Option type not implemented")
            };
        }
    ...
    /// A <see cref="Packet" />
    /// </param>
    public IPv4Packet(ByteArraySegment byteArraySegment, Packet parentPacket) : this(byteArraySegment)
    {
    ...
    public int FragmentFlags
    {
        get
        {
            var fragmentOffsetAndFlags = EndianBitConverter.Big.ToInt16(Header.Bytes,
                                                                        Header.Offset + IPv4Fields
                                                                            .FragmentOffsetAndFlagsPosition);
    ...
    public override int TotalLength
    {
        ...
        set => EndianBitConverter.Big.CopyBytes((ushort)value,
                                                Header.Bytes,
                                                Header.Offset + IPv4Fields.TotalLengthPosition);
    }
    ...
    public bool ValidIPChecksum
    {
        get
        {
            ...
            Log.DebugFormat(HexPrinter.GetString(Header.ActualBytes(),
                                                 0,
                                                 Header.Length));
            ...
            Log.DebugFormat("headerOnesSum: {0}, expectedHeaderOnesSum {1}, returning {2}",
                            ...
                            result);
            Log.DebugFormat("Header.Length {0}",
                            Header.Length);
    ...
    public ushort CalculateIPChecksum()
    {
        ...
        var calculatedChecksum = ChecksumUtils.OnesComplementSum(Header.Bytes,
                                                                 Header.Offset,
                                                                 Header.Length);
    ...
    public override string ToString(StringOutputType outputFormat)
    {
        ...
        if (outputFormat is StringOutputType.Verbose or StringOutputType.VerboseColored)
        {
            ...
            var diffServices = Convert.ToString(DifferentiatedServices,
                                                2).PadLeft(8,
                                                           '0').Insert(4,
                                                                       " ");
            properties.Add("differentiated services",
                           "0x" + DifferentiatedServices.ToString("x").PadLeft(2,
                                                                               '0'));
            properties.Add("",
                           diffServices.Substring(0,
                                                  7) + ".. = [" + (DifferentiatedServices >> 2) + "] code point");
            properties.Add(" ",
                           ".... .." + diffServices[6] + ". = [" + diffServices[6] + "] ECN");
            properties.Add("  ",
                           ".... ..." + diffServices[7] + " = [" + diffServices[7] + "] ECE");
            properties.Add("total length",
                           TotalLength.ToString());
            properties.Add("identification",
                           "0x" + Id.ToString("x") + " (" + Id + ")");
            var flags = Convert.ToString(FragmentFlags,
                                         2).PadLeft(8,
                                                    '0').Substring(5,
                                                                   3);
            properties.Add("flags",
                           "0x" + FragmentFlags.ToString("x").PadLeft(2,
                                                                      '0'));
            properties.Add("   ",
                           flags[0] + ".. = [" + flags[0] + "] reserved");
            properties.Add("    ",
                           "." + flags[1] + ". = [" + flags[1] + "] don't fragment");
            properties.Add("     ",
                           ".." + flags[2] + " = [" + flags[2] + "] more fragments");
            properties.Add("fragment offset",
                           FragmentOffset.ToString());
            properties.Add("time to live",
                           TimeToLive.ToString());
            properties.Add("protocol",
                           Protocol + " (0x" + Protocol.ToString("x") + ")");
            properties.Add("header checksum",
                           "0x" + Checksum.ToString("x") + " [" + (ValidChecksum ? "valid" : "invalid") + "]");
    ...
    public static IPv4Packet RandomPacket()
    {
        ...
        return new IPv4Packet(srcAddress,
                              dstAddress);
    }
